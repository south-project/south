  ...
  <div>
    ...
    <div class="page_view">
      ...
      >
        ...
        <el-table-column prop="reservePrice" label="预设单价(万元/亩)"></el-table-column>
        <el-table-column prop="buildingTypePrice" label="平均单价(万元/亩)"></el-table-column>
...
import {
  queryOtherBuildTypePrice,
  buildValueExpExcelData,
  updateOtherReservePrice,
  batchOtherUpdateReservePrice,
  setAvgAllPrice
} from "@/api/getData";
...
export default {
  ...
  methods: {
    initData(id) {
      ...
      this.param = param;
      queryOtherBuildTypePrice(param).then(res => {
        if (res.code == 200) {
    ...
    //导出数据
    handleDownloadExcel() {
      buildValueExpExcelData(this.param).then(res => {
    ...
    sendMessage(e) {
      ...
      if (!this.coreState) {
        ...
        updateOtherReservePrice(param).then(res => {
          //修改预设单价
          if (res.code == 200) {
            this.$message({
              showClose: true,
        ...
        param.idList = arr;
        batchOtherUpdateReservePrice(param).then(res => {
          //批量修改预设单价
          if (res.code == 200) {
            this.$message({
              showClose: true,
